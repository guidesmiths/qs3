...
function uploadToS3(config, ctx, next) {
    ...
    next(null, function(message, content, cb) {
        ...
        debug(format('Uploading %d bytes to bucket: %s and key: %s', Buffer.byteLength(content), message.qs3.bucket, message.qs3.s3id))
        ...
        s3Client.putObject({
            ...
            debug(format('Error uploading message to bucket: %s and key: %s', message.qs3.bucket, message.qs3.s3id))
