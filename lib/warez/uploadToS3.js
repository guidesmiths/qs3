...
function uploadToS3(config, ctx, next) {
    ...
    next(null, function(flowScope, message, content, cb) {
        ...
        debug(format('Uploading %d bytes to bucket: %s and key: %s', message.content.length, config.bucket, flowScope.qs3.s3id))
        ...
        s3Client.putObject({
            ...
            debug(format('Error uploading message to bucket: %s and key: %s', config.bucket, flowScope.qs3.s3id))
